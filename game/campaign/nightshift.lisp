...
(dialog-text "nightshift/notes-1"
             "the predecessor's notes are practical the way only the desperate are practical. drink before 0603. they log the drinking and worry if it is late. the worry travels upward in reports. spare them the reports. the water is good. it is the one thing here with no second purpose."
...
(dialog-text "nightshift/notes-3"
             "the last page of notes is later than the rest, the pencil pressed harder. when you hear the binder's pages through the wall, that is the log being read, and you are the subject of the sentence being written. it is possible to sleep through this. it took me years. start tonight."
             :next "nightshift/palimpsest")

(dialog-text "nightshift/palimpsest"
             "the notebook is older than its cover. under M-3's pencil, where the rubber has worn the paper soft, an earlier hand shows through, and under that another, the layers going down like wallpaper in an old house. the earliest legible layer is just numbers: in for four. hold for four. out for four."
...
(dialog-text "nightshift/hem-shadows"
             "the hem becomes your social life. you learn the watchers by their shadows along it: M-3's, still as furniture. the day shift's, rocking heel to toe at the hour's end. the new ones, whose shadows lean in too close and jerk back when the glass fogs. you have opinions about all of them. the room files your opinions under breathing."
...
(dialog-text "nightshift/meals"
             "meals arrive during intervals that contain no one: you look away at the lawful moment, and the small table has a tray on it, and the tray is warm. the notes cover this too. do not try to catch the tray. the trying is logged, and worse, it works once, and you will wish it had not."
             :next "nightshift/eleven")

(dialog-text "nightshift/eleven"
             "the eleven minutes are the morning's whole craft. wake at 0552 and rise at 0603, the notes say, because fewer minutes reads as distress and more reads as decline, and eleven is the kept middle, the figure the far side can initial without a report. you lie in the kept middle each morning, holding the facility's nerves steady with your stillness."
             :next "nightshift/count")

(dialog-text "nightshift/count"
             "in the long hours you count the doors. the figure comes out wrong by one, which is correct: the count of this room has been wrong by one since before your handwriting, and the wrongness is load-bearing. you log it in the notebook, in pencil, under the predecessor's last entry. figure disputed. room breathing. all well."
             :next "nightshift/watchers-note")

(dialog-text "nightshift/watchers-note"
             "deep in the notebook, one page is set apart, written calmer than the rest: about the watchers. they are more frightened than you. you have the room. they have the corridor, and the corridor has them. be watchable. it is the only kindness that travels in that direction, and it travels."
...
(dialog-text "nightshift/gift"
             "you stir, and do not wake, and take the next breath the familiar way, deliberately, the way it was once taken at you: a breath like a hand held out through glass. on the far side, a held breath answers it. the gift passes. you are the subject of someone's first line now, and you have made it a kind one."
             :next "nightshift/spelling")

(dialog-text "nightshift/spelling"
             "the new watcher's pencil is slow through the wall, stopping at the hard words, and you can hear the handbook being consulted, pages against pages. you breathe slower on those nights, spacing yourself out like dictation, giving them time to spell you correctly. it is the room side's one vanity: being a fair copy."
             :next "nightshift/audit")

(dialog-text "nightshift/audit"
             "once a year, two grey coats inventory the room around you while you hold the kept middle of the bed. bed, one. night stand, one. glass, one, full. doors, and here the two of them count separately, compare, and write the dispute down with visible relief: figure disputed, room breathing, all well, initials, initials. the wrongness passes its inspection, load-bearing as ever."
...
(dialog-text "nightshift/card-inside"
             "the laminated card is in the drawer where you put it, and you hold it in the dark. IN THE EVENT OF RECURRENCE, REMAIN WHERE YOU ARE. from this side of the glass, the instruction reads differently. it is not a restriction. it is a promise extracted from the room: that remaining is possible, and that where you are will hold still enough to be remained in."
             :next "nightshift/remain-inside")

(dialog-text "nightshift/remain-inside"
             "you remain where you are. the room goes somewhere with you in it, the way a berth goes somewhere with its sleeper, and what passes outside the disputed door passes politely, at the pace of a thing pacing itself, and you breathe the drill, in for four, hold for four, out for four, until the wall is a wall."
             :next "nightshift/morning-after")

(dialog-text "nightshift/morning-after"
             "in the morning the glass is full to the line, refilled per schedule during an interval you would swear contained no one, and the log through the wall reads one line longer than the night you remember. you initial the notebook instead. both records are kept. only one of them is yours."
             :next "nightshift/souvenirs")

(dialog-text "nightshift/souvenirs"
             "some mornings after recurrence the room comes back carrying. a pine needle on the sill. flour dust in the blanket's weave. once, pressed flat under the glass of water, a tram ticket, punched, from a city whose name the inventory disputes. the room travels, and files its travels, and you are the cabinet."
             :next "nightshift/souvenirs-2")

(dialog-text "nightshift/souvenirs-2"
             "you keep them in the drawer with the notebook: needle, dust folded in paper, ticket. the inventory never objects. the drawer was always going to hold this. somewhere in an archive a file gets a line longer each time, and you could write the spine labels yourself now: CROSSING. HILL HOUSE. THIRD DISTRICT. OAKBARROW."
             :next "nightshift/mug")

(dialog-text "nightshift/mug"
             "one tray arrives with a mug on it. grey, designation stenciled, yours, from the staff room locker row you will never stand in again. nobody initials a reason. somebody on the far side simply decided the room side should have its own mug, and fought whatever paperwork that took, and lost the fight quietly, and sent it anyway."
...
(dialog-text "nightshift/m3-watch"
             "M-3 takes the watch himself some rotations, you learn his rings on the curtain rail, slower than anyone's, and his breathing, which is the drill worn so smooth it is just breath now. he was on the room side once. nobody breathes like that from reading a handbook. the drill is what you take with you when they let you out."
...
(dialog-text "nightshift/bad-nights"
             "on the bad nights, and the room side has them, M-3 stands at the wall after hours and reads you the day's log line aloud, low, against every procedure he has ever initialed. subject slept. figure disputed. classification unchanged. read in his voice, through stone, it is not a report. it is a tucking in."
             :next "nightshift/succession")

(dialog-text "nightshift/succession"
             "so the cycle shows you its whole shape at last: subject, then handler, then the long seniority of standing near walls. the second coat in your locker, elbows gone soft. the column of initials that stop being letters and start being yours. the facility does not hire. it rotates."
...
(dialog-text "nightshift/stay-room"
             "you keep the room. the notebook gets a new first page in your hand: NOTES. FOR THE NEXT ONE, which is how the room side says forever. through the wall, a new pencil takes up the log, unschooled, careful, and you drink before 0603, and spare them the report, and the room breathes its disputed figure around you, honest as ever."
             :next "nightshift/end")
...
(dialog-text "nightshift/take-coat"
             "the coat fits the way the older coat in the locker always promised it would. on your first watch from the corridor side you draw the curtain back slowly, rings one at a time, and stand away from the glass, and the sleeper inside stirs, and takes the next breath the familiar way, and you hold yours, per the drill, and the drill holds."
...
(dialog-text "nightshift/walk-out"
             "you walk the painted line out, past the desk, past the sheet with its column of your initials, and the line carries you to a door you have never been shown, and the handle is warm, and on the other side of the door someone has just let go of it, the way someone always has, and you go through anyway, which is the one move the facility never files, because it cannot see past its own doors."
             :next "nightshift/end")
