...
- Concreteness does not freeze canon. A later divergence may still reveal and recontextualize even deep material, and the reveal is branch-relative: one branch can establish that a path was a simulation all along while another holds it was real all along. Each is that branch's own canon, not one truth shared across paths; the reveal should still feel like it was always so, never like reality changed.
...
- Never fan one story beat into a row of parallel sibling variants. A cluster of near-identical nodes at one fixed slot (`something-a`, `something-b`, ...) is a crossroads the player can feel even without seeing the graph. Distribute the differences instead: pick at most one variation per visit through the store, and surface it as a binary variant of whichever ordinary beat owns that detail diegetically.
- Keep branch entries diegetic and concise. Use at most one ordinary, scene-continuing beat when the transition needs cover; do not add filler nodes to slow the player down.
- Fanning is fine when it is a legitimate one-of-N choice inside a narrative: in-fiction decisions may branch freely and carry consequences. What must never fan is canon: no single spot where parallel options each catapult the player into a different narrative or genre (one pick makes you a containment researcher, the next a fantasy ghost). Canon-splitting divergence happens one gentle entry at a time, in-scene and without padding. Cross-path echo variants must stay rare, single, and woven into existing beats.
- Choices should express actions, attention, speech, or decisions available to the current player character. Do not use a choice to decide facts outside the character's control, such as which unrelated person happens to meet them, unless the character has an explicit in-world way to cause that fact or the branch has established reality-warping as a character power.
...
- Keep individual dialogue nodes short enough to look good in the typewriter UI. If a real scene beat needs more text, split it into several short chained nodes; do not add padding nodes just to slow a transition down.
- When the project owner asks for more padding or a path to be less compressed, interpret that as playable/story substance: more minigame reuse, more events, more encounters, more concrete story beats, and more authored consequences. Do not answer that request with filler bridge prose, vague connective tissue, or deliberately mysterious paragraph padding.
...
- The JRPG path can be corny and stereotypical early. The hidden long-term idea is that the demon lord may turn out to be the player from another path, but this must not be foreshadowed in the early route. JRPG stats, inventory, companion, route choices, and combat results should use the shared dialog store.
- The built-in JRPG demon-lord route is one authored spine, not a canonical endpoint for all JRPG play. Keep meaningful choice and mod hook points so players and modders can diverge from that route into their own trees.
