...
- Pad every branch trigger. Keep at least one ordinary, scene-continuing beat between a triggering choice and the first node a path family owns, so entering a path reads as the same moment carrying on, not a doorway between narratives.
- Fanning is fine when it is a legitimate one-of-N choice inside a narrative: in-fiction decisions may branch freely and carry consequences. What must never fan is canon: no single spot where parallel options each catapult the player into a different narrative or genre (one pick makes you a containment researcher, the next a fantasy ghost). Canon-splitting divergence happens one gentle entry at a time, padded and in-scene. Cross-path echo variants must stay rare, single, and woven into existing beats.
