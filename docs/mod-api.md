...
A minigame is a black box: it reads its inputs from the node's config
plist, plays, and finishes by returning an outcome id — a dialog target,
usually one it was handed in config, sometimes a static one like the
node's success or failure target. The engine never interprets config or
outcomes. The node's `:outcomes` list is just the ids the minigame may
finish at beyond success and failure, declared so tools like the graph
lint can see every destination without understanding the minigame.
...
(dialog-minigame "my-mod/crossroads"
                 ...
                 :config (list :fog 0.5
                               :left "my-mod/meadow"
                               :right "my-mod/cliff")
                 :outcomes (list "my-mod/meadow" "my-mod/cliff"))
```

Inside the minigame, read inputs with
`(minigame-config-value node :left)` and finish with
`(finish-minigame-node node target)`. A nil target falls back to the
node's failure target, then its success target.
...
`(register-minigame-session-kind :my-mod/maze 'my-maze-session)`. The
engine keeps one session per node visit with the node's config inside
(`session-config-value`), and `finish-minigame-node` clears it. Sessions
read and write the shared dialog store through their own handle:

```lisp
(setf (session-store-value session "my-mod-tries")
      (1+ (session-store-value session "my-mod-tries" 0)))
```
...
See `game/rogue/delve/core.lisp` for a full example: a reusable, multi-floor
...
(dialog-set-minigame-outcomes "my-mod/crossroads"
                              (list "my-mod/meadow" "my-mod/cliff"))
```
